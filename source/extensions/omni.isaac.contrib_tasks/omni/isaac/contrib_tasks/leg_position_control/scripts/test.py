...
def main():
    ...
    # Get trajectory
    points_x, points_z = generate_circular_trajectory(center=torch.tensor([0.55, -0.5]), radius=0.15, num_points=1000)
    idx = 0
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            obs[:,-19:-16] = torch.tensor([points_x[idx], 0.9, points_z[idx]], device=obs.device)
            # agent stepping
            actions = policy(obs)
            # env stepping
